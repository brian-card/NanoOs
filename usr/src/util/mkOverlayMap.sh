...
echo '    .overlay.blockDevice = NULL,' >> "${outputFile}"
echo '    .overlay.startBlock = 0,' >> "${outputFile}"
echo '    .overlay.numBlocks = 0,' >> "${outputFile}"
echo '    .version = ((uint32_t) 0 << 24) | ((uint32_t) 0 << 16) | ((uint32_t) 1 << 8) | ((uint32_t) 0 << 0),' >> "${outputFile}"
