...
Another big hurdle was figuring out how to manage pipes from the shell.  Normally, the shell would `fork()` a bunch of copies, each of which would `exec()` part of the chain.  That's not possible in NanoOs at the moment because I don't have a way to `fork()`.  Fortunately, there's an alternative:  `posix_spawn()`.  Of course, going that route came with its own set of dedicated system calls, so I had to implement those as well, but at least there was a clean "Unix" way of doing things for that one.

And, Oh! the bugs that got created when I tried to implement pipes for the first time were something to behold!!  I was pretty tired when I was wrapping up the implementation and inadvertently created a situation where a process could send its output to the scheduler.  The scheduler, of course, had no idea what to do with that and silently threw it away.  That, in turn, resulted in the message that had been sent being reused by another process while the original was still waiting on a response.  What a mess that was!!  It took me the majority of a day to track down and fix that one!

I had one last crash to fix after that.  The way that pipes work, the last command in the chain takes over the shell's command slot.  In the event that launching that command fails, the stdin file descriptor will be linked to the next-to-last command in the chain.  That command is killed as part of the cleanup, so the stdin for the shell process is closed at the end of the cleanup.  I hadn't counted on that in my original implementation of the shell and I was getting `NULL` returned from `fgets()` in the shell's `main()`.  Once I got that taken care of, I was able to exit the shell gracefully and restart from a login prompt.

Still, in the end, I succeeded.  Now, unfortunately, I'm running out of memory when trying to pipe more than two commands together, but that's an issue to work out differently.  There were a **LOT** of things that I uncovered that need to be fixed before going any further.  One of those things is the legacy messaging system.  It's currently consuming too much RAM and not providing enough messages to the processes, so fixing that should go a long way toward fixing some of the memory issues.  There's a laundry list of other things too but I'll cover those in some future post.
